    ...
    fetch(config.url+'api/user',{
        ...
        headers:
          {'Accept':'application/json',
            ...
            this.setState({networkInfo:'Please Check your internet and try again'})
          }
  }
  logout(){
    AsyncStorage.getItem('authenticated')
              ...
              if (value==1) {
                ...
                if (data !== null) {
                    if (JSON.parse(data)) {
                      var authData = JSON.parse(data)
                      if(authData.access_token && authData.refresh_token && authData.token_type && authData.refresh_token && authData.expires_in){
                        this.sendLogout($token = authData.token_type+' '+authData.access_token)
                      }
                      else{
                        Actions.login({type:'reset'})
                      }
    ...
    return (
      ...
      {(!this.state.isConnected)?<View style={styles.loadBar}><Text style={styles.white}>Please check your Internet! </Text></View>:null}
        ...
        <View style={styles.center}>
          <View>
            <View style={styles.center}><Icon name="ios-contact" size={200}/></View>
            <Text style={styles.indent}><Text style={styles.labeled}>Names</Text>: {this.state.names}</Text>
            <Text style={styles.indent}><Text style={styles.labeled}>Phone</Text>: {this.state.phone}</Text>
            <Text style={styles.indent}><Text style={styles.labeled}>Email</Text>: {this.state.email}</Text>
          </View>
          <View>
            <Text style={styles.indent}>{this.state.auctions}</Text>
          </View>
          <TouchableHighlight onPress={()=>this.logout()} style={styles.buttonTouch} underlayColor={colors.accent}><View style={styles.button}><Text style={styles.white}>Logout</Text></View></TouchableHighlight>
        </View>
        </View>
        </ScrollView>
      </View>
